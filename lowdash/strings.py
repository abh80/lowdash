from .arrays import shuffle, chunks as array_chunks
...
def shorten(string: str, length: int, sep="...") -> str:
    ...
    if(not isinstance(length, int)):
        raise TypeError(
            "[lowdash.strings] Argument length must be of type int")
    if(not isinstance(sep, str)):
        raise TypeError(
            "[lowdash.strings] Argument sep must be of type string")
